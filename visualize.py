...
def visualize():
    ...
    dataset = Dataset(
        ...
        include_autocorr_fingerprint=True,
        include_extended_connectivity_fingerprint=True,
        include_extended_reduced_graph_fingerprint=True,
        include_estate_fingerprint=True,
        include_functional_groups_fingerprint=True,
        include_ghose_crippen_fingerprint=True,
        include_klekota_roth_fingerprint=True,
        include_laggner_fingerprint=True,
        include_layered_fingerprint=True,
        include_lingo_fingerprint=True,
        include_map_fingerprint=True,
        include_mhfp_fingerprint=True,
        include_mqns_fingerprint=True,
        include_pattern_fingerprint=True,
        include_pubchem_fingerprint=True,
        include_smiles_extended_connectivity_fingerprint=True,
        include_vsa_fingerprint=True,
    )
    # We compute the features without augmentation
    _scalers, (train_x, train_y), _ = dataset.primary_split(augment=True)
    ...
    for feature_set_name, features in tqdm(
        ...
        for i, (label_name, indices, top_labels) in enumerate(
            ...
            for j, label in enumerate(top_labels):
                ...
                labels.append(label)
            ax[i].legend(handles, labels, loc="upper right", prop={"size": 8})
