...
class Dataset(Hashable):
    ...
    def multi_label() -> bool:
        """Return whether the dataset is multi-label or not."""
    ...
    def primary_split(
        ...
        leaf_layer_counter: Counter = counters[self.layered_dag().leaf_layer_name]
        least_common_leaf_labels: np.ndarray = np.fromiter(
            (
                leaf_layer.index(
                    labelled_smiles.least_common_label(
                        leaf_layer_counter, self.layered_dag().leaf_layer_name
                    )
                )
